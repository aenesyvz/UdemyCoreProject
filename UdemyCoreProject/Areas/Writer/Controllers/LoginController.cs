...
namespace UdemyCoreProject.Areas.Writer.Controllers
{
    ...
    public class LoginController : Controller
    {
        ...
        public async Task<IActionResult> Index(UserLoginViewModel userLoginViewModel)
        {
            ...
            {
                var result = await _signInManager.PasswordSignInAsync(userLoginViewModel.UserName, userLoginViewModel.UserName, true, true);
                if (result.Succeeded)
                {
                ...
                {
                    ModelState.AddModelError("", "Hatalı Kullanıc Adı veya Şifre Girdiniz!");
                }
            }
            return View();
        }
